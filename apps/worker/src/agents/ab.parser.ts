 ...
 * 2. Try to extract table-like lines with "<pos>  <qty> x <desc>  <price>".
 * 3. Try German date near each line for confirmedDeliveryAt.
 *
 * This is intentionally simple. The real pipeline should call the PDF
 * parser sidecar and, for heterogeneous layouts, fall back to an LLM
 * extractor with a typed schema. Keep this deterministic parser as a
 * working baseline and as a safety net when LLMs are unavailable.
 */
