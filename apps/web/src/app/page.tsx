  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <KpiCard label="Zu prüfen" value={snap.data?.toReviewDocs ?? '—'} tone="warn" />
            ...
            <tbody>
              {(scorecards.data ?? []).map((s) => (
                <tr key={s.supplierId} className="border-t border-border">
                  <td className="py-1">{s.supplierId}</td>
...
function KpiCard({ label, value, tone }: { label: string; value: any; tone?: 'warn' | 'danger' }) {
